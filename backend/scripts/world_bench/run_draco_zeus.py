...
def zeus_answer(
    *,
    ...
    """Call ZeusCode power+research; retry on empty / stub / 5xx / 429 (A6 flakiness)."""
    body = {
        "model": "zeuscode",
        "stream": False,
        "messages": [
        ...
        "zeus": {
            "mode": "power",
            # thinking off for bench throughput; research pack still runs.
            "thinking": False,
            # Force research×3→Sonnet glue (no GPT doer) on every DRACO task
            "research": True,
        },
    }
    ans = ""
    ...
    for attempt in range(1, attempts + 1):
        ...
        stubbish = (
            "Synthesized from three researcher notes" in ans
            or ans.strip().startswith("# Research report\n\n## Summary\nSynthesized")
        )
        # Real research answers are long; short stubs / empty → retry.
        ok_ans = bool(ans.strip()) and status == 200 and not stubbish and len(ans.strip()) >= 1200
        if ok_ans:
            return ans, meta
        if attempt < attempts:
            # 502/503/429/empty/stub — back off and retry whole Zeus turn
            delay = min(25.0, 2.5 * attempt)
            print(
                f"         retry gen {attempt}/{attempts - 1} "
                f"status={status} chars={len(ans)} stub={int(stubbish)} sleep={delay:.1f}s",
                flush=True,
            )
            time.sleep(delay)
    return ans, meta
    ...
    *,
    ...
    for attempt in range(5):
        ...
        if status == 200 and text.strip():
            break
        # 429/502/503 — back off harder so parallel judges don't stampede A6.
        time.sleep(min(20.0, 2.0 * (attempt + 1) + (0.5 * attempt)))
    # Transport death: do NOT pretend UNMET — resume/rejudge must catch this.
...
def score_task(criteria_results: list[dict[str, Any]], rubric: list[dict[str, Any]]) -> float:
    if not rubric:
        return 0.0
    pairs = list(zip(rubric, criteria_results))
    if not pairs:
        return 0.0
    # Denominator = weights of judged criteria only (capped rubric slice).
    tw = sum(float(c.get("weight") or 1.0) for c, _r in pairs) or 1.0
    got = 0.0
    for c, r in pairs:
        if r.get("verdict") == "MET":
            got += float(c.get("weight") or 1.0)
    # Cap at 100 — front-loaded high weights must not exceed 100%.
    return min(100.0, 100.0 * got / tw)
...
def main() -> int:
    ...
    ap.add_argument(
        ...
        type=int,
        default=6,
        help="Parallel judge threads (1 = sequential). Speeds DRACO wall-time ~2× on judge half.",
    )
    args = ap.parse_args()
        ...
        for line in ckpt_path.read_text(encoding="utf-8").splitlines():
            ...
            if row0.get("score_pct") is not None:
                ...
                if not ans0 and chars0 <= 0:
                    ...
                    continue
                # Judge transport death: empty verdict/reason or 5xx/0 → rejudge
                crit0 = row0.get("criteria") or []
                if ans0 and crit0:
                    empty_r = sum(1 for c in crit0 if not str(c.get("reason") or "").strip())
                    empty_v = sum(
                        1 for c in crit0 if str(c.get("verdict") or "").upper() not in ("MET", "UNMET")
                    )
                    bad_http = sum(
                        1
                        for c in crit0
                        if int(c.get("http_status") or 0) == 0
                        or int(c.get("http_status") or 0) >= 500
                        or c.get("judge_failed")
                    )
                    if (
                        empty_r >= max(3, len(crit0) // 2)
                        or empty_v >= max(3, len(crit0) // 2)
                        or bad_http >= max(3, len(crit0) // 2)
                    ):
                        print(
                            f"[draco] resume: will rejudge dead-judge row {rid[:8]}… "
                            f"(empty_reason={empty_r} empty_verdict={empty_v} bad_http={bad_http})",
                            flush=True,
                        )
                        row0 = dict(row0)
                        row0["score_pct"] = None
    ...
    for i, task in enumerate(tasks, 1):
        ...
        if answer and not args.skip_judge and rubric:
            workers = max(1, min(int(args.judge_workers or 1), len(rubric)))
            print(
                f"         judge 1/{len(rubric)}… (workers={workers})",
                flush=True,
            )
            crit_out: list[dict[str, Any]] = [{} for _ in rubric]

            def _one(ci_c: tuple[int, dict[str, Any]]) -> tuple[int, dict[str, Any]]:
                ci, c = ci_c
                j = judge_criterion(
                    base=args.base,
                    key=args.key,
            ...
            if workers == 1:
                for ci, c in enumerate(rubric):
                    _, row_j = _one((ci, c))
                    crit_out[ci] = row_j
                    if ci == 0 or (ci + 1) % 10 == 0 or ci + 1 == len(rubric):
                        print(f"         judge {ci + 1}/{len(rubric)}…", flush=True)
            else:
                with ThreadPoolExecutor(max_workers=workers) as pool:
                    futs = [pool.submit(_one, (ci, c)) for ci, c in enumerate(rubric)]
                    done_n = 0
                    for fut in as_completed(futs):
                        ci, row_j = fut.result()
                        crit_out[ci] = row_j
                        done_n += 1
                        if done_n == 1 or done_n % 10 == 0 or done_n == len(rubric):
                            print(f"         judge {done_n}/{len(rubric)}…", flush=True)
            row["criteria"] = crit_out
            dead = sum(
                1
                for c in crit_out
                if c.get("judge_failed")
                or str(c.get("verdict") or "").upper() not in ("MET", "UNMET")
                or int(c.get("http_status") or 0) >= 500
                or int(c.get("http_status") or 0) == 0
            )
            if dead >= max(3, len(crit_out) // 2):
                row["score_pct"] = None
                print(
                    f"         JUDGE DEAD ({dead}/{len(crit_out)}) — leave unscored for resume",
                    flush=True,
                )
            else:

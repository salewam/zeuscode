#!/usr/bin/env python3
"""STO skill bake: ultra agents=4 with MIXED cheapest models (1 per provider).

Not 4× the same model — different manufacturers on max team mode.
Output: /tmp/sto-mixed-cheap-bake/
"""
...
PASS = "GeminiOnly2026!"
OUT = Path("/tmp/sto-mixed-cheap-bake")

# Mixed manufacturers on ultra/agents=4.
# Upstream 500 today: grok-4-3, often claude-haiku on heavy FE prompts.
# FE gets strongest worker (gemini-3-flash); flash on design/backend; haiku on tests.
MIXED_CHEAP = [
    "gemini-2.5-flash",  # Google
    "gemini-3-flash",    # Google FE (stable)
...
PROMPT = (
    ...
    f"Микс-моделей bake #{int(time.time()) % 97}."
)
...
def main() -> None:
    ...
    with httpx.Client(base_url=BASE, timeout=30.0, verify=False) as c:
        ...
        print("MIXED team_models", MIXED_CHEAP, flush=True)
        print(
            "Expected: judge=gpt-5-2; FE=gemini-*-flash; others=haiku/grok/flash (Kie retry)",
            flush=True,
        )
        ...
        proj = c.post(
            "/projects",
            headers=headers,
            json={"title": f"STO mixed-cheap ultra4 {int(time.time())}"},
        )
        ...
        for e in events:
            ...
            if e.get("type") in ("synth_start", "synth_done", "review_start", "review_done"):
                print(
                    f"  {e.get('type')}: model={e.get('model')} role={e.get('role')}",
                    flush=True,
                )

        print("\n=== ROLE → MODEL (this is the proof of mix) ===", flush=True)
        ...
        report = {
            "focus": "ultra_agents4_mixed_providers",
            "team_models_requested": MIXED_CHEAP,
